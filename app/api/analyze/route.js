...
import { detectSeaportMarketplaceTrade } from "../../../lib/seaportMarketplaceDetection";
...
export async function POST(req) {
  ...
  const marketplaceDetection = canDetectHigherLevelActions ? detectSeaportMarketplaceTrade({ receipt, chainId: chain.id, walletActivity }) : null;
  const nftLifecycleDetection = canDetectHigherLevelActions ? detectNftLifecycleAction({ tx, nftTransfers }) : null;
  const classification = bridgeDetection || swapDetection || marketplaceDetection || nftLifecycleDetection || baseClassification;
  const detections = { bridge: bridgeDetection, swap: swapDetection, marketplace: marketplaceDetection, nftLifecycle: nftLifecycleDetection };
