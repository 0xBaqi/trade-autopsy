...
function nftLabel(transfer) {
  ...
  const standard = transfer.standard === "ERC1155" ? "ERC-1155 item" : "ERC-721 NFT";
...
export function buildDeterministicAnalysis(data) {
  ...
  switch (classification.type) {
    ...
    case "NFT_TRANSFER": {
      ...
      if (outgoing.length === 1 && incoming.length === 0) return { verdict: "clean", summary: `You sent ${nftLabel(outgoing[0])} to another address.`, why: `The receipt contains a standard ${outgoing[0].standard} transfer event showing this item movement from your wallet. This proves the transfer, but does not by itself prove a sale or purchase. You paid ${fee} in network fees.`, tip: "Confirm the recipient and token ID match what you intended to transfer." };
      if (incoming.length === 1 && outgoing.length === 0) return { verdict: "clean", summary: `You received ${nftLabel(incoming[0])}.`, why: `The receipt contains a standard ${incoming[0].standard} transfer event showing this item movement into your wallet. This proves the transfer, but does not by itself prove a purchase or mint. You paid ${fee} in network fees.`, tip: "Check the collection contract and token ID before assuming where it came from or what it is worth." };
      return { verdict: "clean", summary: `This transaction moved ${data.nftTransferCount || data.nftTransfers?.length || 0} NFT/item${(data.nftTransferCount || data.nftTransfers?.length) === 1 ? "" : "s"}.`, why: `Standard ERC-721/ERC-1155 transfer events prove the item movements. Trade Autopsy is not labeling them as a sale, purchase, or mint without separate evidence. You paid ${fee} in network fees.`, tip: "Review the token IDs, quantities, and addresses in the evidence before assigning a higher-level meaning to the transaction." };
    }
    case "TOKEN_APPROVAL": {
    ...
    case "CONTRACT_INTERACTION": {
      ...
      const nftActivities = counts.NFT_TRANSFER_ACTIVITY || 0;
      ...
      if (nftActivities > 0) {
        const nftText = `${nftActivities} ERC-721/ERC-1155 item movement${nftActivities === 1 ? "" : "s"} ${nftActivities === 1 ? "was" : "were"} also recorded`;
        movementSummary = movementSummary.endsWith("safely.")
          ? `${nftActivities} ERC-721/ERC-1155 item movement${nftActivities === 1 ? " was" : "s were"} recorded during this contract interaction.`
          : `${movementSummary.replace(/\.$/, "")}, and ${nftText}.`;
      }
      if (transferActivities > 1 || nftActivities > 1 || knownActivities > 1) return { verdict: "warning", summary: movementSummary, why: `This was a composite smart-contract transaction with ${transferActivities} fungible-token movement${transferActivities === 1 ? "" : "s"} and ${nftActivities} ERC-721/ERC-1155 item movement${nftActivities === 1 ? "" : "s"}. We can verify those movements, but the evidence does not safely prove that the whole transaction was one specific action such as a swap, bridge, or NFT purchase. You paid ${fee} in network fees.`, tip: "Review each asset movement and the contract call together before deciding what the transaction was intended to do." };
