...
export function buildDeterministicAnalysis(data) {
  ...
  switch (classification.type) {
    ...
    case "NFT_PURCHASE": {
      const market = classification.marketplace || {};
      const payment = formatAsset(market.payment);
      const nfts = Array.isArray(market.nfts) && market.nfts.length ? market.nfts : market.nft ? [market.nft] : [];
      if (market.fulfillmentCount > 1 && nfts.length > 1) {
        return { verdict: "clean", summary: `You purchased ${nfts.length} NFTs for ${payment} through Seaport.`, why: `${market.fulfillmentCount} verified Seaport 1.6 OrderFulfilled events map one-to-one to the ${nfts.length} NFT movements into your wallet, and their decoded consideration totals exactly ${payment}. This proves a multi-order marketplace purchase without guessing from transfer direction alone. You also paid ${fee} in network fees.`, tip: "Open Technical evidence to review each fulfilled order, NFT, and its decoded consideration." };
      }
      const item = walletNftLabel(market.nft);
      return { verdict: "clean", summary: `You purchased ${item} for ${payment} through Seaport.`, why: `A verified Seaport 1.6 OrderFulfilled event proves marketplace fulfillment, ${item} entered your wallet, and ${payment} left your wallet as consideration. These independent facts support a purchase classification. You also paid ${fee} in network fees.`, tip: "Confirm the NFT contract and token ID match the item you intended to buy." };
    }
    case "NFT_SALE": {
      const market = classification.marketplace || {};
      const payment = formatAsset(market.payment);
      const nfts = Array.isArray(market.nfts) && market.nfts.length ? market.nfts : market.nft ? [market.nft] : [];
      const extraOut = [...(data?.walletActivity?.nativeOut || []), ...(data?.walletActivity?.fungibleOut || [])].filter((asset) => {
        try { return BigInt(asset?.rawAmount ?? "0") > 0n; } catch { return false; }
      });
      const feeNote = extraOut.length ? " Other asset transfers also left your wallet in the same transaction, so this incoming amount is described as gross receipt rather than net proceeds." : "";
      if (market.fulfillmentCount > 1 && nfts.length > 1) {
        return { verdict: "clean", summary: `You sold ${nfts.length} NFTs and received ${payment} through Seaport.`, why: `${market.fulfillmentCount} verified Seaport 1.6 OrderFulfilled events map one-to-one to the ${nfts.length} NFT movements out of your wallet, and their decoded gross consideration totals exactly ${payment}.${feeNote} You paid ${fee} in network fees.`, tip: "Open Technical evidence to review each fulfilled order and use the full asset flows when calculating net proceeds." };
      }
      const item = walletNftLabel(market.nft);
      return { verdict: "clean", summary: `You sold ${item} and received ${payment} through Seaport.`, why: `A verified Seaport 1.6 OrderFulfilled event proves marketplace fulfillment, ${item} left your wallet, and ${payment} entered your wallet as sale consideration.${feeNote} You paid ${fee} in network fees.`, tip: "Use the full asset-flow evidence if you need to calculate net proceeds after marketplace fees or other outgoing transfers." };
    }
    case "NFT_MINT": {
