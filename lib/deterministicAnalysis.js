...
function nftMovementForSender(data) { const sender = data?.from?.toLowerCase(); const transfers = Array.isArray(data?.nftTransfers) ? data.nftTransfers : []; return { outgoing: transfers.filter((t) => t?.from?.toLowerCase() === sender), incoming: transfers.filter((t) => t?.to?.toLowerCase() === sender) }; }
...
export function buildDeterministicAnalysis(data) {
  ...
  switch (classification.type) {
    ...
    case "NFT_TRANSFER": { const { outgoing, incoming } = nftMovementForSender(data); if (outgoing.length === 1 && incoming.length === 0) return { verdict: "clean", summary: `You sent ${nftLabel(outgoing[0])} to another address.`, why: `A standard ${outgoing[0].standard} transfer event proves this item left your wallet. It does not by itself prove a sale or purchase. You paid ${fee} in network fees.`, tip: "Confirm the recipient and token ID match what you intended to transfer." }; if (incoming.length === 1 && outgoing.length === 0) return { verdict: "clean", summary: `You received ${nftLabel(incoming[0])}.`, why: `A standard ${incoming[0].standard} transfer event proves this item entered your wallet. It does not by itself prove a purchase or mint. You paid ${fee} in network fees.`, tip: "Check the collection contract and token ID before assuming where it came from or what it is worth." }; return { verdict: "clean", summary: walletSummary(data) || `This transaction moved ${data.nftTransferCount || 0} NFT/items.`, why: `Standard ERC-721/ERC-1155 transfer events prove the wallet movements, but not their economic meaning. You paid ${fee} in network fees.`, tip: "Review the token IDs, quantities, and addresses before assigning a higher-level meaning." }; }
    case "NFT_MINT": {
