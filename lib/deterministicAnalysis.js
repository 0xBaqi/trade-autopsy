function formatDisplayAmount(amount) {
  ...
  // Keep useful precision without dumping raw token decimals into layman-facing copy.
  // Large values need fewer decimals; small values keep enough significant digits.
...
export function buildDeterministicAnalysis(data) {
  ...
  switch (classification.type) {
    ...
    case "SWAP": {
      ...
      return {
        verdict: "clean",
        summary: `You swapped ${gave} for ${received}.`,
        why: `The transaction used a known swap function, ${gave} left your wallet, and ${received} came back during the same successful transaction. You paid ${fee} in network fees.`,
        tip: "Check that the amount you received matches what you expected from the swap.",
      };
    }
