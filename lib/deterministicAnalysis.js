function formatDisplayAmount(amount) {
  ...
  if (roundedWhole === 0n && !roundedFraction && originalIsNonZero) {
...
export function buildDeterministicAnalysis(data) {
  ...
  if (!data?.success || classification.type === "FAILED_TRANSACTION") {
    return { verdict: "failed", summary: "This transaction failed, so the action you tried to make did not go through.", why: `The blockchain marked the transaction as failed. You may still have paid ${fee} in network fees for the attempt.`, tip: "Before trying again, check the transaction details to see why it failed." };
  }

  switch (classification.type) {
    case "NATIVE_TRANSFER":
      return { verdict: "clean", summary: `You sent ${data.value} ${data.chain.symbol} to another address.`, why: `The blockchain shows a direct ${data.chain.symbol} transfer with no extra contract action. You paid ${fee} in network fees.`, tip: "For future transfers, double-check the receiving address and network fee before sending." };
    case "ERC20_TRANSFER": {
      const movement = outAsset ? formatAsset(outAsset) : "a token";
      return { verdict: "clean", summary: `You sent ${movement} to another address.`, why: `The blockchain recorded a token transfer from your wallet. You paid ${fee} in network fees.`, tip: "Before sending tokens again, double-check the token and receiving address." };
    }
    case "TOKEN_APPROVAL": {
    ...
    case "SWAP": {
      ...
      const gave = formatAsset(swapOut);
      const received = formatAsset(swapIn);
      return { verdict: "clean", summary: `You swapped ${gave} for ${received}.`, why: `The transaction used a known swap function, ${gave} left your wallet, and ${received} came back during the same successful transaction. You paid ${fee} in network fees.`, tip: "Check that the amount you received matches what you expected from the swap." };
    }
    case "CONTRACT_CREATION":
      return { verdict: "clean", summary: "This transaction created a new smart contract on the blockchain.", why: `The transaction created a contract instead of sending funds to an existing address. You paid ${fee} in network fees.`, tip: "If you expected to create a contract, confirm the new contract address before using it." };
    case "CONTRACT_INTERACTION": {
      const counts = activityCounts(data?.activities);
      const transferActivities = counts.TOKEN_TRANSFER_ACTIVITY || 0;
      const knownActivities = (counts.SWAP_ACTIVITY || 0) + (counts.APPROVAL_ACTIVITY || 0);
      ...
      if (transferActivities > 1 || knownActivities > 1) {
        return {
          verdict: "warning",
          summary: movementSummary,
          why: `This was a composite smart-contract transaction with ${transferActivities} observed token movements. We can verify those movements, but the evidence does not safely prove that the whole transaction was one specific action such as a swap or bridge. You paid ${fee} in network fees.`,
          tip: "Review each token movement and the contract call together before deciding what the transaction was intended to do.",
        };
      }
