...
export function classifyTransaction({ tx, receipt, tokenTransfers = [], nftTransfers = [] }) {
  ...
  const nftTransferCount = Array.isArray(nftTransfers) ? nftTransfers.length : 0;
  ...
  if (nftTransferCount > 0 && selector && NFT_TRANSFER_SELECTORS.has(selector)) {
    return {
      type: "NFT_TRANSFER",
      confidence: "HIGH",
      reason: "The call uses a standard ERC-721/ERC-1155 transfer selector and the receipt emitted matching NFT transfer evidence.",
    };
  }
