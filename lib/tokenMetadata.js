...
function decodeSymbolResult(result) {
  ...
  // Some older tokens return bytes32 instead of the standard ABI dynamic string.
  if (hex.length === 64) {
    const zeroIndex = hex.match(/00/)?.index;
    const contentHex = zeroIndex === undefined ? hex : hex.slice(0, zeroIndex - (zeroIndex % 2));
    return cleanSymbol(decodeUtf8(contentHex));
  }

  // Standard ABI string return: offset word, then length word, then UTF-8 bytes.
...
async function safeEthCall(chain, to, data) {
  try {
    return await rpcCall(chain, "eth_call", [{ to, data }, "latest"]);
  } catch {
    return null;
  }
}

export async function resolveTokenMetadata(chain, tokenAddress) {
  const [decimalsResult, symbolResult] = await Promise.all([
    safeEthCall(chain, tokenAddress, DECIMALS_SELECTOR),
    safeEthCall(chain, tokenAddress, SYMBOL_SELECTOR),
  ]);
