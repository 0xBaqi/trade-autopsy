...
function buildEvidence(data) {
  return {
    ...
    evidenceLimits: {
      ...
      transactionInitiatorMayDifferFromAffectedNftOwner: true,
      nftTransferEventsProveMovementNotEconomicMeaning: true,
      nftLifecycleClassificationMayProveMintOrBurnFromZeroAddressSemantics: true,
...
- Lead the summary with what entered and left the sender wallet, except when deterministic lifecycle evidence explicitly proves an operator-initiated NFT action affecting another owner.
...
- NFT transfer evidence proves movement only. Never call it a purchase, sale, mint, burn, gift, marketplace trade, or payment relationship unless supplied deterministic classification explicitly proves that meaning.
- If classification.nft.relationship says senderIsAssetOwner is false, distinguish the transaction initiator/operator from the affected NFT owner. Never say the NFT entered or left the sender's wallet.
