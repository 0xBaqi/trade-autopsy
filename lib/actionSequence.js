...
function orderingLabel(orderingSource, stepCount) {
  if (orderingSource === "COMMAND_INDEX") return "VERIFIED_COMMAND_ORDER";
  if (orderingSource) return "VERIFIED_EVIDENCE_ORDER";
  return stepCount > 1 ? "MULTIPLE_PRIMARY_ACTIONS_ORDER_UNPROVEN" : "SINGLE_PRIMARY_ACTION";
}
...
export function reconstructActionSequence({ classification, activities }) {
  ...
  return {
    version: 2,
    ordering: orderingLabel(orderingSource, steps.length),
    orderingSource,
    steps,
