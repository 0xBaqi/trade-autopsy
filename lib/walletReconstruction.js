...
function nftEntry(transfer, direction) {
  const standard = transfer.standard === "ERC1155" ? "ERC1155" : "ERC721";
  return {
    assetType: standard,
    standard,
 ...
 *
 * ERC-1155 TransferBatch events may contain zero-value entries. Those remain
 * available in raw NFT evidence, but they are not counted as asset movements.
 */
export function reconstructWalletActivity({ tx, assetFlows, nftTransfers = [] }) {
  ...
  for (const transfer of Array.isArray(nftTransfers) ? nftTransfers : []) {
    if (!positiveNftQuantity(transfer)) continue;
