...
const FACILITATOR_URL = "https://facilitator.x402.org"; // OKX uses standard facilitator
...
// Check if request is coming from our own frontend (internal bypass)
// This allows the web UI to work for free while external agent-to-agent calls require payment
export function isInternalRequest(request) {
  const internalKey = request.headers.get("X-INTERNAL-KEY");
  const expectedKey = process.env.INTERNAL_API_KEY || "trade-autopsy-internal";
