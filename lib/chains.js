...
export async function rpcTransactionBundle(chain, hash) {
  ...
  // Prefer one provider for both records.
  for (const url of chain.rpcs) {
    try {
  ...
  // Fall back to independent providers, but accept the pair only after their
  // transaction hashes (and block hashes when present) prove they describe the same record.
  try {
    const [tx, receipt] = await Promise.all([
