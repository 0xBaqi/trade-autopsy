...
function rpcUrlsFor(chain) {
  const configured = chain.id === "ethereum"
    ? [process.env.ETHEREUM_RPC_URL, process.env.ETHEREUM_RPC_URL_2]
    : [];
  return [...new Set([...configured.filter(Boolean), ...(chain.rpcs || [])])];
}
...
async function bundleFromProvider(url, hash) {
  const tx = await rpcCallOne(url, "eth_getTransactionByHash", [hash], 8500);
  if (!tx || !sameHash(tx.hash, hash)) throw new Error("Transaction unavailable from provider");

  const receipt = await rpcCallOne(url, "eth_getTransactionReceipt", [hash], 8500);
  if (!receipt || !sameHash(receipt.transactionHash, hash)) throw new Error("Receipt unavailable from provider");

  const blocksAgree = !tx.blockHash || !receipt.blockHash || sameHash(tx.blockHash, receipt.blockHash);
  if (!blocksAgree) throw new Error("Transaction and receipt block hashes disagree");
...
export async function rpcTransactionBundle(chain, hash) {
  ...
  // Query providers concurrently, while keeping each provider's tx/receipt
  // reads sequential. This avoids a single slow or incomplete public RPC
  // blocking the rest of the provider pool.
  try {
    return await Promise.any(urls.map((url) => bundleFromProvider(url, hash)));
  } catch (e) {
    lastError = e;
  }

  // Public RPCs may index the transaction and receipt differently. As a final
  // fallback, allow the two records to come from different providers, then
  // verify hash and block consistency before accepting the pair.
  try {
  ...
  return { tx: null, receipt: null, error: lastError?.message || null };
}
