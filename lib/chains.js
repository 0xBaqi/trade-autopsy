...
// Analysis needs a transaction and its receipt to describe the exact same on-chain record.
// Fetch both from one provider before accepting the result so two public RPCs cannot disagree
// and produce a false "not found" after chain detection already succeeded.
  ...
  for (const url of chain.rpcs) {
    try {
      const [tx, receipt] = await Promise.all([
        rpcCallOne(url, "eth_getTransactionByHash", [hash]),
        rpcCallOne(url, "eth_getTransactionReceipt", [hash]),
      ]);
