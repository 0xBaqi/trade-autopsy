 ...
 * transfers and network fees are not included in these flow totals.
 */
export function reconstructAssetFlows({ tx, receipt, chain, tokenTransfers = [] }) {
  ...
  if (nativeValue > 0n) {
    ...
    if (nativeSelfTransfer) {
      addAmount(incoming, key, nativeValue, template);
      net.set(key, { rawAmount: 0n, template });
    } else {
