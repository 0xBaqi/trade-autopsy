function transferActivity(transfer) {
  return {
    ...
    evidence: {
      ...
      logIndex: Number.isInteger(transfer.logIndex) ? transfer.logIndex : null,
    },
  };
}

function nftTransferActivity(transfer) {
  return {
    type: "NFT_TRANSFER_ACTIVITY",
    confidence: "HIGH",
    evidence: {
      ...
      batchIndex: Number.isInteger(transfer.batchIndex) ? transfer.batchIndex : null,
      logIndex: Number.isInteger(transfer.logIndex) ? transfer.logIndex : null,
    },
  };
}
...
function swapActivity(detection) {
  ...
  return {
    ...
    evidence: {
      ...
      logIndex: Number.isInteger(detection.swap.logIndex) ? detection.swap.logIndex : null,
...
function marketplaceActivity(detection) {
  if ((detection?.type !== "NFT_PURCHASE" && detection?.type !== "NFT_SALE") || !detection.marketplace) return null;
  const marketplace = detection.marketplace;
  return {
    type: "NFT_MARKETPLACE_ACTIVITY",
    confidence: detection.confidence || "HIGH",
    evidence: {
      ...
      logIndex: Number.isInteger(marketplace.logIndex) ? marketplace.logIndex : null,
      nft: marketplace.nft || null,
      payment: marketplace.payment || null,
      paymentPerspective: marketplace.paymentPerspective || null,
      evidenceLimits: marketplace.evidenceLimits || null,
    },
  };
}
...
export function buildActivityEvidence({ classification, detections, tokenTransfers, nftTransfers }) {
  const activities = [];
  ...
  const marketplace = marketplaceActivity(detections?.marketplace);
  if (marketplace) activities.push(marketplace);
  ...
  for (const transfer of nftTransfers || []) activities.push(nftTransferActivity(transfer));
  return activities;
}
