...
function universalRouterEvidence(tx, selector) {
...
export function detectSwapClassification({ tx, receipt, assetFlows }) {
  ...
  const universalCommands = universalRouterEvidence(tx, selector);
  ...
  const reason = isUniversalRouter
    ? "The call targets a verified PancakeSwap Universal Router, its decoded command stream contains a swap command, and sender-side evidence shows one asset leaving while a different asset enters."
    : v3Method
