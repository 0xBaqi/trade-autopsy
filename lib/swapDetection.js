...
function swapCompletionLogIndex(tx, assetsIn, tokenTransfers) {
  const sender = typeof tx?.from === "string" ? tx.from.toLowerCase() : null;
  if (!sender) return null;

  const incomingTokens = new Map();
  for (const asset of assetsIn || []) {
    if (asset?.assetType !== "ERC20" || typeof asset.tokenAddress !== "string") continue;
    incomingTokens.set(asset.tokenAddress.toLowerCase(), String(asset.rawAmount ?? ""));
  }
  if (incomingTokens.size === 0) return null;

  const matches = (tokenTransfers || []).filter((transfer) => {
    if (!Number.isInteger(transfer?.logIndex)) return false;
    if (typeof transfer?.to !== "string" || transfer.to.toLowerCase() !== sender) return false;
    const token = typeof transfer?.tokenAddress === "string" ? transfer.tokenAddress.toLowerCase() : null;
    if (!token || !incomingTokens.has(token)) return false;
    const expected = incomingTokens.get(token);
    return !expected || String(transfer.rawAmount) === expected;
  });

  if (matches.length === 0) return null;
  return Math.max(...matches.map((transfer) => transfer.logIndex));
}

export function detectSwapClassification({ tx, receipt, assetFlows, chainId, tokenTransfers }) {
  ...
  return {
    ...
    swap: {
      ...
      logIndex: swapCompletionLogIndex(tx, assetsIn, tokenTransfers),
      assetsOut,
      assetsIn,
